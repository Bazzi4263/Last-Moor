...
public class CreaturePatrol : Patrol 
{
    ...
    IEnumerator SearchRoomCoroutine(Enemy enemy)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            if (enemy.currentRoom == MapManager.Instance.playerCurrentRoom)
            {
                enemy.SetDestination(roomCollider.bounds.center + new Vector3(Random.Range(-extents.x + 0.5f, extents.x - 0.5f), 0, Random.Range(-extents.z + 0.5f, extents.z - 0.5f)), false);
                yield return new WaitUntil(enemy.isArriveDestination);
            }
        }
        ...
        for (int i = 3; i > 0; i--)
        {
            if (enemy.currentRoom == MapManager.Instance.playerCurrentRoom)
            {
                ...
                enemy.SetDestination(playerNearPos, false);
                yield return new WaitUntil(enemy.isArriveDestination);
            }
        }
...
public class CreatureTrace : Trace
{
    ...
    public override void Enter(Enemy enemy)
    {
        if (!((Creature)enemy).isFind && enemy.state != EnemyStates.Hit)
        {
            ...
            enemy.Invoke(() => ((Creature)enemy).isFind = false, 10f);
        ...
        if (!hasShouted)
        {
            ...
            enemy.Invoke(() => hasShouted = false, 10f);
        }
        else
        {
            enemy.StartNav();
